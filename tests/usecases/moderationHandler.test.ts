  ...
  describe('Severity-based responses', () => {
    ...
    it('should provide strict response for severe profanity', () => {
      ...
      expect(response.responseType).toBe('strict');
              expect(response.response).toMatch(/токсична|трында|останній|подих|аб'юзивна|паща|морда|квиток|пиздеця|трындітиме|мразь|заїбав|слово|асфальт|закатаю|⛔|токсити|сука|пизда|дебіл|думаєш|здохни|бані/i);
    });
  });
