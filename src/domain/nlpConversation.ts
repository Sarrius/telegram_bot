...
export interface ConversationResponse {
  response: string;
  confidence: number;
  shouldUseHumorousReply: boolean;
...
export class NLPConversationEngine {
  ...
  private readonly contextTimeoutMs = 30 * 60 * 1000; // 30 minutes

  // Ukrainian language patterns
  private readonly ukrainianPatterns = {
  ...
  async generateConversationalResponse(context: ConversationContext): Promise<ConversationResponse> {
    const now = Date.now();
  ...
  private detectIntent(message: string, language: 'uk' | 'en' | 'mixed'): string {
  ...
  private generateUkrainianResponse(context: ConversationContext, intent: string): string {
    ...
    const intentResponses = ukrainianResponses[intent as keyof typeof ukrainianResponses] || ukrainianResponses.general_chat;
    return intentResponses[Math.floor(Math.random() * intentResponses.length)];
  }

  private generateEnglishResponse(context: ConversationContext, intent: string): string {
    const { userName } = context;
    ...
    const intentResponses = responses[intent as keyof typeof responses] || responses.general_chat;
    return intentResponses[Math.floor(Math.random() * intentResponses.length)];
  }

  private applyBotPersonality(response: string, context: ConversationContext, intent: string, language: 'uk' | 'en' | 'mixed'): string {
  ...
  private shouldUseHumor(context: ConversationContext, intent: string): boolean {
    const humorIntents = ['joke_request', 'general_chat', 'greeting'];
    return humorIntents.includes(intent) || Math.random() < 0.4;
  }

  private updateUserContext(context: ConversationContext): void {
  ...
  private cleanOldContexts(now: number): void {
